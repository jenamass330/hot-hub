...
const allActMovies = (
  ...
  {castInfo.slice(10 ,n).map((crewDetails) => (
    ...
    <Poster src={`https://image.tmdb.org/t/p/w200${crewDetails.poster_path}`} />
    </Link>
    <div style={{
      fontSize: "15px",
      width: "90px",
      textAlign: "center",
    }}>{crewDetails.title} as {crewDetails.character}</div>
    </Wrap>
    </>
  ))}
  </Grid>
  ...
  return (
    <>
      {(productionInfo.profile_path === null) ?  (
        ...
        <DetailWrapper>
          <Photo src={`https://image.tmdb.org/t/p/w200${productionInfo.profile_path}`}
          ></Photo>
          <Details>
            <Name>{productionInfo.name}</Name>
            <Birth>{productionInfo.birthday}</Birth>
      ...
      <CreditWrapper>
      <PlayedIn>Movies {productionInfo.name} was a crew member for: </PlayedIn>
      <Grid>
        {production.slice(0,10).map((act) => (
          <>
          <Wrap>
            <Link to={`/movies/${act.id}`}>
              <Poster src={`https://image.tmdb.org/t/p/w200${act.poster_path}`} />
            </Link>
            <div style={{
              fontSize: "15px",
              width: "90px",
